...
impl Bitmap {
    pub fn empty(width: isize, height: isize) -> Self {
        Self {
    ...
    pub fn get(&self, x: isize, y: isize) -> (u8,u8,u8) {
        (self.red.get(x, y),self.green.get(x, y), self.blue.get(x, y))
    }

    pub fn get_mut(&self, x: isize, y: isize) -> (u8,u8,u8) {
        (self.red.get(x, y),self.green.get(x, y), self.blue.get(x, y))
    }

    pub fn push(&mut self, col: (u8,u8,u8)) {
        self.red.data.push(col.0);
        self.green.data.push(col.1);
        self.blue.data.push(col.2);
    }

    pub fn iter_chain(&self) -> impl Iterator<Item = &u8> {
        self.red
            .iter()
    ...
    pub fn iter_zip(&self) -> impl Iterator<Item = (u8, u8, u8)> + '_ {
        self.red
            .iter()
            .copied()
            .zip(self.green.iter().copied())
            .zip(self.blue.iter().copied())
            .map(|((r, g), b)| (r, g, b))
    }
...
impl Channel {
    ...
    pub fn get_mut(&mut self, x: isize, y: isize) -> &mut u8 {
        self.data.get_mut((y * self.width + x) as usize).unwrap()
    }

    pub fn get_lin(&self, i: usize) -> u8 {
...
pub fn read_img_to_bitmap(input: image::DynamicImage) -> Bitmap {

    let image = input.into_rgb8();
    let width = image.width();
    let height = image.height();

    let mut bitmap = Bitmap::empty(width as isize, height as isize);

    for y in 0..height {
        for x in 0..width {
            let col = image.get_pixel(x, y);

            bitmap.red.data.push(col.0[0]);
            bitmap.green.data.push(col.0[1]);
            bitmap.blue.data.push(col.0[2]);
        }
    }
